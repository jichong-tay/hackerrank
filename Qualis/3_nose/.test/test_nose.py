...
@pytest.mark.parametrize(
    "source, expected", [("nosetests.xml", 10)], ids=["test_testcase_result"]
)
def test_nose_testcase_result(get_xml_root_element, expected):
    test_case_success = (
        int(get_xml_root_element.attrib["tests"])
        - int(get_xml_root_element.attrib["failures"])
        - int(get_xml_root_element.attrib["errors"])
        - int(get_xml_root_element.attrib["skip"])
    )
    assert (
        int(get_xml_root_element.attrib["tests"]) == expected
